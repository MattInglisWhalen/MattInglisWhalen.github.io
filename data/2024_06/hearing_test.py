...
def binary_equivalent_accuracy(acc_c, n_cats):
    ...
    """
    nbea = acc_c ** (1 / (2*n_cats))
    return nbea

def bad_binary_equivalent_accuracy(acc_c, n_cats):
    """
...
def run_and_plot_accuracies():
    ...
    # accuracy plotting
    fig, ax = plt.subplots()
    sns.scatterplot(x=cats, y=accuracies)
    ax.errorbar(x=cats,
                y=accuracies,
                yerr=[ (a*(1-a)/N_SAMPLES)**0.5 for a in accuracies],
    ...
    sns.scatterplot(x=cats, y=[binary_equivalent_accuracy(a, n) for a, n in zip(accuracies,cats)])
    plt.show()
